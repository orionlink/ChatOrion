    ...
    _unread_msg_total_count(0)
{
    ...
    clearAction->setIcon(QIcon(":/pic/close_transparent.png"));
    ui->search_edit->addAction(clearAction, QLineEdit::TrailingPosition);

    QObject::connect(ui->search_edit, &QLineEdit::textChanged, [clearAction](const QString &text)
    {
        if (!text.isEmpty()) {
            clearAction->setIcon(QIcon(":/pic/close_search.png"));
        } else {
            clearAction->setIcon(QIcon(":/pic/close_transparent.png")); // 文本为空时，切换回透明图标
        }
    ...
    QObject::connect(clearAction, &QAction::triggered, [this, clearAction]
    {
        ...
        clearAction->setIcon(QIcon(":/pic/close_transparent.png")); // 清除文本后，切换回透明图标
